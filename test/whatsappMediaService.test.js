...
test('descriptor detects downloadable image payload', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'image', image: { id: 'img-123', caption: 'Fachada principal', mime_type: 'image/jpeg', sha256: 'sha-img-1' } });
...
test('descriptor handles document metadata', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'document', document: { id: 'doc-777', filename: 'escritura.pdf', caption: 'escritura completa', mime_type: 'application/pdf', sha256: 'sha-doc-1' } });
  assert.equal(descriptor.shouldDownload, true);
...
test('normalizeMimeTypeForPolicy strips parameters', () => {
...
test('isAllowedDownload accepts audio/ogg with codecs parameter', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'audio', audio: { id: 'aud-1', mime_type: 'audio/ogg; codecs=opus' } });
  const policy = isAllowedDownload(descriptor);
  assert.equal(policy.allowed, true);
  assert.equal(policy.reason, null);
});

test('isAllowedDownload rejects unsupported mime', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'audio', audio: { id: 'aud-2', mime_type: 'audio/wav; codecs=whatever' } });
  const policy = isAllowedDownload(descriptor);
  assert.equal(policy.allowed, false);
  assert.equal(policy.reason, 'skipped_unsupported_mime');
});

test('evaluateGraphAndDescriptorMimePolicy allows octet stream with allowed audio hint', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'audio', audio: { id: 'a1', mime_type: 'audio/ogg; codecs=opus' } });
  const ev = evaluateGraphAndDescriptorMimePolicy({ mime_type: 'application/octet-stream', url: 'https://example.com/x' }, descriptor);
...
test('evaluateGraphAndDescriptorMimePolicy reads metadata.mimeType camelCase', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'audio', audio: { id: 'a1', mime_type: 'audio/ogg; codecs=opus' } });
  const ev = evaluateGraphAndDescriptorMimePolicy({ mimeType: 'application/octet-stream' }, descriptor);
  assert.equal(ev.allowed, true);
});

test('evaluateGraphAndDescriptorMimePolicy rejects concrete unsupported types', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'audio', audio: { id: 'a1', mime_type: 'audio/webm' } });
...
test('uniqueNormalizedMimeHints dedupes and preserves order', () => {
  const hints = uniqueNormalizedMimeHints({ mime_type: 'audio/ogg; codecs=opus' }, { mimeType: 'audio/ogg; codecs=opus' });
  assert.deepEqual(hints, ['audio/ogg']);
});

test('descriptor marks video/mp4 downloadable in Sprint C', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'video', video: { id: 'vid-1', mime_type: 'video/mp4', caption: 'Recorrido' } });
...
test('descriptor marks sticker webp downloadable in Sprint C', () => {
  const descriptor = getInboundMediaDescriptor({ type: 'sticker', sticker: { id: 'stk-1', mime_type: 'image/webp' } });
