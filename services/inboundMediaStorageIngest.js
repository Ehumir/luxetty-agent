...
const SCHEDULED_MEDIA_TYPES = new Set(['image', 'audio', 'voice', 'document', 'sticker', 'video']);
...
function captionPresentFromMessage(message = {}) {
  ...
  if (t === 'document') return Boolean(String(message?.document?.caption || '').trim() || String(message?.document?.filename || '').trim());
...
function buildWhatsappMediaRecord({ waMessageType, metaMediaId, mimeType, byteSize, storageBucket, storagePath, downloadStatus, ingestedAt, errorCode, filename, captionPresent }) {
  return {
    schema_version: 1,
...
async function applyWhatsappMediaMetadata(supabase, messageId, whatsappMedia) {
  const { data: row, error: selErr } = await supabase.from('conversation_messages').select('metadata').eq('id', messageId).maybeSingle();
  if (selErr) return { ok: false, error: selErr };
  if (!row) return { ok: false, error: new Error('message_row_missing') };
  const prev = row.metadata;
  const base = prev && typeof prev === 'object' && !Array.isArray(prev) ? { ...prev } : {};
  const { error: updErr } = await supabase.from('conversation_messages').update({ metadata: { ...base, whatsapp_media: whatsappMedia } }).eq('id', messageId);
  if (updErr) return { ok: false, error: updErr };
  return { ok: true };
}

async function shouldSkipIngestAlreadyStored(supabase, messageId) {
  const { data: row, error } = await supabase.from('conversation_messages').select('metadata').eq('id', messageId).maybeSingle();
...
async function runInboundMediaIngest({ supabase, logEvent, conversationId, inboundMessageId, message }) {
  ...
  if (!supabase || !conversationId || !inboundMessageId || !message) return;
  const waType = message.type || null;
  if (!SCHEDULED_MEDIA_TYPES.has(waType)) return;

  if (String(process.env.PERSEO_INBOUND_MEDIA_STORAGE_ENABLED || '').toLowerCase() !== 'true') {
    log('perseo_inbound_media_ingest_skipped', { reason: 'flag_disabled', message_id: inboundMessageId, wa_message_type: waType });
    return;
  }
  if (await shouldSkipIngestAlreadyStored(supabase, inboundMessageId)) {
    log('perseo_inbound_media_ingest_skipped', { reason: 'already_stored', message_id: inboundMessageId });
    return;
  }

  log('perseo_inbound_media_ingest_scheduled', { message_id: inboundMessageId, wa_message_type: waType });
  ...
  if (!hasGraphMediaToken()) {
    await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
      waMessageType: waType, metaMediaId, mimeType: descriptor.mimeType || null, byteSize: null,
      storageBucket: null, storagePath: null, downloadStatus: 'skipped_token_missing', ingestedAt: ingestedNow,
      errorCode: 'skipped_token_missing', filename, captionPresent,
    }));
    log('perseo_inbound_media_ingest_terminal', { message_id: inboundMessageId, download_status: 'skipped_token_missing' });
    return;
  }
  ...
  if (!descriptor.shouldDownload) {
    await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
      waMessageType: waType, metaMediaId, mimeType: descriptor.mimeType || null, byteSize: null,
      storageBucket: null, storagePath: null, downloadStatus: 'skipped_unsupported', ingestedAt: ingestedNow,
      errorCode: descriptor.reason || 'skipped_unsupported', filename, captionPresent,
    }));
    log('perseo_inbound_media_ingest_terminal', { message_id: inboundMessageId, download_status: 'skipped_unsupported' });
    return;
  }

  await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
    waMessageType: waType, metaMediaId, mimeType: descriptor.mimeType || null, byteSize: null,
    storageBucket: null, storagePath: null, downloadStatus: 'pending', ingestedAt: null,
    errorCode: null, filename, captionPresent,
  }));
  ...
  if (!resolved.success || !resolved.buffer) {
    const st = resolved.download_status === 'skipped_unsupported_mime' ? 'skipped_unsupported_mime' : 'failed';
    const errCode = String(resolved.error_code || (st === 'failed' ? 'resolve_failed' : 'skipped_unsupported_mime'));
    await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
      waMessageType: waType, metaMediaId, mimeType: finalMime, byteSize: resolved.size_bytes != null ? resolved.size_bytes : null,
      storageBucket: null, storagePath: null, downloadStatus: st, ingestedAt: terminalTime,
      errorCode: errCode, filename, captionPresent,
    }));
    log('perseo_inbound_media_ingest_failed', { message_id: inboundMessageId, download_status: st, error_code: errCode });
    return;
  }
  ...
  if (upErr) {
    await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
      waMessageType: waType, metaMediaId, mimeType: finalMime, byteSize: resolved.size_bytes != null ? resolved.size_bytes : null,
      storageBucket: BUCKET_ID, storagePath: null, downloadStatus: 'failed', ingestedAt: terminalTime,
      errorCode: String(upErr.message || 'storage_upload_failed').slice(0, 200), filename, captionPresent,
    }));
    log('perseo_inbound_media_ingest_failed', { message_id: inboundMessageId, download_status: 'failed', error_code: 'storage_upload_failed' });
    return;
  }

  const byteSize = resolved.size_bytes != null ? Number(resolved.size_bytes) : resolved.buffer.length;
  await applyWhatsappMediaMetadata(supabase, inboundMessageId, buildWhatsappMediaRecord({
    waMessageType: waType, metaMediaId, mimeType: finalMime, byteSize,
    storageBucket: BUCKET_ID, storagePath, downloadStatus: 'stored', ingestedAt: terminalTime,
    errorCode: null, filename, captionPresent,
  }));
  log('perseo_inbound_media_ingest_stored', { message_id: inboundMessageId, byte_size: byteSize, wa_message_type: waType });
}
...
function scheduleInboundMediaIngest({ supabase, logEvent, conversationId, inboundMessageId, message }) {
  setImmediate(() => {
    runInboundMediaIngest({ supabase, logEvent, conversationId, inboundMessageId, message }).catch((err) => {
      ...
      if (typeof logEvent === 'function') logEvent('perseo_inbound_media_ingest_fatal', { message_id: inboundMessageId, err: safe });
      else console.error('perseo_inbound_media_ingest_fatal', safe);
    });
  });
}
